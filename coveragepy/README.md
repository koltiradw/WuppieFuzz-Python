# WuppieFuzz-Python — coverage.py backend

This backend gathers Python coverage through the
[coverage.py](https://coverage.readthedocs.io/) package and exposes it to
WuppieFuzz in [LCOV format](https://ltp.sourceforge.io/coverage/lcov/geninfo.1.php).
Out of the box coverage.py only writes coverage information to file once the
Python process terminates. As we work with never-ending services and wish to
gather coverage information whilst running the service, this module patches
coverage.py's LCOV reporter and serves the report over a TCP connection on
demand.

It is shipped as a `sitecustomize.py` that Python imports automatically when
placed on the `site-packages` path. There is also a newer
[sys.monitoring backend](../sys-monitoring/README.md) with no third-party
dependencies.
...
```console
$ cp coveragepy/sitecustomize.py <path to site-packages>
```
...
Copy (and modify) the `.coveragerc` found in the repository root to the location
where you are launching your Python PUT.
